...
def main():
    ...
    <h2 style="background-color:red;">Old Car Price Prediction</h2>
    </body>
    
    """
    st.markdown(html,unsafe_allow_html=True)
    p1=st.slider("enter year of Purchase: ",1995,2019)
    p2=st.number_input("enter no of kilometers driven:")
    p3=st.selectbox("enter fuel_type: ",['Petrol','Diesel','LPG'])
    ...
    p4=st.selectbox('Enter car comany: ',company)
    car_name=['Hyundai Santro Xing', 'Mahindra Jeep CL550', 'Hyundai Grand i10',
    ...
    p5=st.selectbox('enter your car name: ',car_name)
    
    if st.button('Predict'):
        model=joblib.load('linearmodel')
        df=pd.DataFrame([[p5,p4,p1,p2,p3]],columns=['name','company','year','kms_driven','fuel_type'])
        value=model.predict(df)
        
        st.balloons()
        st.success('your predicted price:{}'.format(round(value[0],2)))
